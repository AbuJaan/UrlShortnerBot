export function printWatermark(apiToken) {
  const status = "AUTOMATIC";
  const botName = "🔗 URL Shortner";
  const text = "BilloXD";
  ...
  console.log(`\x1b[1m\x1b[36m            🔑 API Token: ${apiToken.slice(0, 2)}*****${apiToken.slice(-2)}`); // Masked API token
  console.log('\x1b[1m\x1b[36m║                                            ║');
